...
def create_custom_short_url(url: str, custom_short_url: str) -> dict:
    if not save_url(url, custom_short_url):
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=SHORT_URL_EXISTS_WARNING.format(short_url=custom_short_url))
...
def create_generated_short_url(url: str) -> dict:
    ...
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=ERROR_MAX_RETRIES)
